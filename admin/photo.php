 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: photo.php 17217 2011-01-19 06:29:08Z liubo $
 */
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    /* 模板赋值 */
    $goods_ur = array('' => $_LANG['01_photo_list'], 'virtual_card'=>$_LANG['50_virtual_card_list']);
    $ur_here = ($_REQUEST['act'] == 'list') ? $goods_ur[$code] : $_LANG['11_goods_trash'];
    $smarty->assign('ur_here', $ur_here);

    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'photo.php?act=list', 'text' => $_LANG['01_photo_list']);
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        'photo_list.htm' : (($_REQUEST['act'] == 'trash') ? 'photo_trash.htm' : 'group_list.htm');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    /* 模板赋值 */
    $smarty->assign('code',    $code);
    $smarty->assign('ur_here', $is_add ? (empty($code) ? $_LANG['02_photo_add'] : $_LANG['51_virtual_card_add']) : ($_REQUEST['act'] == 'edit' ? $_LANG['edit_goods'] : $_LANG['copy_goods']));
...
//        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 3, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
...
//        }
    ...
    $product_list_url = $GLOBALS['ecs']->url()."admin/photo.php?act=product_list&goods_id=".$goods_id;
    ...
    {
        $link[0] = array('href' => 'photo.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    {
        $link[] = array('href' => 'photo.php?act=trash', 'text' => $_LANG['11_goods_trash']);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $tpl = $is_delete ? 'photo_trash.htm' : 'photo_list.htm';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
        ...
        $url = 'photo.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
    ...
    $url = 'photo.php?act=query&' . str_replace('act=restore_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    clear_cache_files();
    $url = 'photo.php?act=query&' . str_replace('act=drop_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    {
        $link[] = array('href' => 'photo.php?act=list', 'text' => $_LANG['cannot_found_goods']);
    ...
    if (empty($goods))
    {
        $link[] = array('href' => 'photo.php?act=list', 'text' => $_LANG['01_photo_list']);
        sys_msg($_LANG['cannot_found_goods'], 1, $link);
    }
    ...
    if (empty($attribute))
    {
        $link[] = array('href' => 'photo.php?act=edit&goods_id=' . $goods_id, 'text' => $_LANG['edit_goods']);
    ...
    $smarty->assign('action_link',  array('href' => 'photo.php?act=list', 'text' => $_LANG['01_photo_list']));
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    if ($result)
    {
        ...
        $url = 'photo.php?act=product_query&' . str_replace('act=product_remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    if ($insert)
    {
        $link[] = array('href' => 'photo.php?act=add', 'text' => $_LANG['02_photo_add']);
        $link[] = array('href' => 'photo.php?act=list', 'text' => $_LANG['01_photo_list']);
        $link[] = array('href' => 'photo.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
    else
    {
        $link[] = array('href' => 'photo.php?act=list&uselastfilter=1', 'text' => $_LANG['01_photo_list']);
        $link[] = array('href' => 'photo.php?act=edit&goods_id=' . $product['goods_id'], 'text' => $_LANG['edit_goods']);
        $link[] = array('href' => 'photo.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
...
{
    /* 定义返回 */
    $link[] = array('href' => 'photo.php?act=product_list&goods_id=' . $_POST['goods_id'], 'text' => $_LANG['item_list']);
...
function list_link($is_add = true, $extension_code = '')
{
    ...
    {
        $text = $GLOBALS['_LANG']['01_photo_list'];
    }
...
function add_link($extension_code = '')
{
    ...
    {
        $text = $GLOBALS['_LANG']['02_photo_add'];
    }
