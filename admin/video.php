...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    /* 模板赋值 */
    $goods_ur = array('' => $_LANG['01_video_list'], 'virtual_card'=>$_LANG['50_virtual_card_list']);
    $ur_here = ($_REQUEST['act'] == 'list') ? $goods_ur[$code] : $_LANG['03_video_trash'];
    $smarty->assign('ur_here', $ur_here);

    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'video.php?act=list', 'text' => $_LANG['01_video_list']);
    ...
    $smarty->assign('list_type',    $_REQUEST['act'] == 'list' ? 'video' : 'trash');
    ...
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, 2);//1=实物；0=虚拟；2=视频
    ...
    $htm_file = ($_REQUEST['act'] == 'list') ?
        'video_list.htm' : (($_REQUEST['act'] == 'trash') ? 'video_trash.htm' : 'group_list.htm');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    /* 模板赋值 */
    $smarty->assign('code',    $code);
    $smarty->assign('ur_here', $is_add ? (empty($code) ? $_LANG['02_video_add'] : $_LANG['51_virtual_card_add']) : ($_REQUEST['act'] == 'edit' ? $_LANG['edit_goods'] : $_LANG['copy_goods']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $pub_id = empty($_POST['pub_id']) ? 1 : intval($_POST['pub_id']);//默认是系统管理员
...
//        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, video_url, is_outer_video_url, pub_id)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 2, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$video_url', '$is_outer_video_url', '$pub_id')";
//        }
    }
    else
    {
...
//        }
    ...
    {
        $link[0] = array('href' => 'video.php?act=product_list&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    {
        $link[] = array('href' => 'video.php?act=trash', 'text' => $_LANG['03_video_trash']);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    $goods_list = goods_list($is_delete, 2);
    ...
    $smarty->assign('record_count', $goods_list['record_count']);
    $smarty->assign('page_count',   $goods_list['page_count']);
    $smarty->assign('list_type',    $is_delete ? 'trash' : 'video');
    $smarty->assign('use_storage',  empty($_CFG['use_storage']) ? 0 : 1);
    ...
    $tpl = $is_delete ? 'video_trash.htm' : 'video_list.htm';
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
        ...
        admin_log(addslashes($goods_name), 'trash', 'video'); // 记录日志
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    if ($insert)
    {
         $link[] = array('href' => 'video.php?act=add', 'text' => $_LANG['02_video_add']);
         $link[] = array('href' => 'video.php?act=list', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'video.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
    else
    {
         $link[] = array('href' => 'video.php?act=list&uselastfilter=1', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'video.php?act=edit&goods_id=' . $product['goods_id'], 'text' => $_LANG['edit_goods']);
         $link[] = array('href' => 'video.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
...
function list_link($is_add = true, $extension_code = '')
{
    ...
    {
        $text = $GLOBALS['_LANG']['01_video_list'];
    }
...
function add_link($extension_code = '')
{
    ...
    {
        $text = $GLOBALS['_LANG']['02_video_add'];
    }
